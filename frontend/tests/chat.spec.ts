...
test.describe('Chat E2E', () => {
  test('fluxo básico de chat', async ({ page }) => {
    ...
    await page.waitForTimeout(500);
    await page.screenshot({ path: 'test-results/02-nova-conversa.png' });

    // 3. Envia uma mensagem
    await page.getByPlaceholder('Digite sua mensagem...').fill(TEST_MESSAGE);
    await page.locator('#send-message').click();
    await page.waitForTimeout(500);
    await page.screenshot({ path: 'test-results/03-mensagem-enviada.png' });
    ...
    await page.screenshot({ path: 'test-results/04-resposta-assistente.png' });
